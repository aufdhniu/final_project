...
class Ball:
    ...
    def time_to_hit_paddle_side(self,paddle):
        ...
        paddle_left_edge = paddle.location[1] - paddle.width/2
        paddle_right_edge = paddle.location[1] + paddle.width/2
        if paddle_left_edge - self.size <= self.y + (self.vy*dt) <= paddle_right_edge + self.size:
            return dt
        else:
            return math.inf
